...
@Component
@Priority(-10000)
class EtcMountConfig implements VfsMountConfig
{
    @Override
    public List<VfsMountEntry> getEntries()
    {
        VfsMountEntry etc = new VfsMountEntry("/etc", "CLASSPATH");
        etc.setProperties(new HashMap<>());
        etc.getProperties().put("classpath", "/BRIDJE-INF/etc");
        return Arrays.asList(new VfsMountEntry[]
        {
            etc
        });
    }
